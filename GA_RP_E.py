...
class RampProblem(ElementwiseProblem):
    ...
    def __init__(self, service = 'Enquiry'):
        ...
        #super().__init__(vars=vars, n_obj=2, n_ieq_constr=2, xl = [40, 0.75, 0.00000001], xu = [100, 1, 0.000001])
        super().__init__(vars=vars, n_obj=2, n_ieq_constr=2, xl = [0.75, 0.0000001], xu = [1, 0.00001])
        self.service = service
        self.scoresPP = []
        self.scoresRP = []

    def _evaluate(self, x, out, *args, **kwargs):
        command = "Rscript detect_ramp_E.R {} {} {} ".format(self.service, x["pi"], x["mu"])
        #print(command)
        ...
        if scorePP > 0 and scoreRP > 0:
          ...
          print("GA {} {} {} {} {}".format(self.service, x["pi"], x["mu"], scorePP, scoreRP))
...
def main():
    ...
    if "GA" in args.alg:
      ...
      with open(csv_data_path, 'w', newline='') as f:
        ...
        csv_writer.writerow(['PP','RP','pi','mu'])
        ...
        with open(csv_exe_path, 'w', newline='') as g:
          ...
          for s in services:
            ...
            for r in range(args.repeat):
                ...
                #print(myZ)
            plot.save('ParetoFront/Ericsson/ParetoFront_GA_RP_E'+s+'.pdf')
           ...
           for s in services:
             ...
             for r in range(args.repeat):
                ...
                for i in range(args.size):
                    ...
                    command = "Rscript detect_ramp_E.R {} {} {}".format(s, pi, mu)
                    #print(command)
                    result = run(command, stdout=PIPE, stderr=PIPE, universal_newlines=True, shell=True)
                    #print("c",result.stdout)
                    scorePP = parse_result(result.stdout.split("\n"))[0]
                    scoreRP = parse_result(result.stdout.split("\n"))[1]
                    #print("scorePP=", scorePP,"ScoreRP=",scoreRP)
                    if scorePP > 0 and scoreRP > 0:
                      scoresPP.append(scorePP)
                      scoresRP.append(scoreRP)
                      print("RAND {} {} {} {}  {}".format(s, scorePP, scoreRP, pi, mu))
                      temp="{:f} {:f} {:f} {:f} ".format(scorePP, scoreRP, pi, mu)
                      temp=temp.split()
